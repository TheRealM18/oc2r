...
public final class WebImportExportCardItemDevice extends AbstractItemRPCDevice implements DocumentedDevice {
    ...
    private static final class ImportFileRequest {
        ...
        public final WeakReference<WebImportExportCardItemDevice> Device;

        private ImportFileRequest(final WebImportExportCardItemDevice device) {
    ...
    public WebImportExportCardItemDevice(final ItemStack identity, final TerminalUserProvider userProvider) {
        super(identity, "web_import_export");
    ...
    public static void setImportedFile(final int id, final String name, final byte[] data) {
        synchronized (importingDevices) {
            ...
            if (request != null) {
                final WebImportExportCardItemDevice device = request.Device.get();
                if (device != null) {
    ...
    public boolean requestImportFile(@Parameter("url") String external_url, @Parameter("name")  String file_name) {
        if (state != State.IDLE) {
            throw new IllegalStateException("invalid state");
        }
...
//        }
        try {
            ...
            System.out.print("Received external name :  " + external_url);
            System.out.print("Received file name :  " + file_name);
        } catch (IOException e) {
